        ...
        <h1>任务详细信息:</h1>
          ...
          <table width="700" border="0" cellpadding="0" cellspacing="0" class="table">
            <tr>
              ...
              <td width="579" colspan="5">${lgtaskinfo.taskName}</td>
            </tr>
            <tr>
              <td class="tdcolor">任务描述</td>
              <td colspan="5">${lgtaskinfo.taskDesc}</td>
            </tr>
            <tr>
              <td class="tdcolor">开始时间</td>
              <td width="15%"><fmt:formatDate value="${lgtaskinfo.beginDate }" pattern="yyyy-MM-dd"/></td>
              <td width="20%" class="tdcolor">结束时间</td>
              <td width="50%" colspan="3"><p><fmt:formatDate value="${lgtaskinfo.endDate }" pattern="yyyy-MM-dd"/></p>              </td>
            </tr>
            <tr>
              <td class="tdcolor">实际开始时间</td>
              <td><fmt:formatDate value="${lgtaskinfo.realBeginDate }" pattern="yyyy-MM-dd"/></td>
              <td class="tdcolor">实际结束时间</td>
              <td colspan="3"><fmt:formatDate value="${lgtaskinfo.realEndDate }" pattern="yyyy-MM-dd"/></td>
            </tr>
            <tr>
              <td class="tdcolor">实施人</td>
              <td>${lgtaskinfo.mp_emp.employeeName}</td>
	    ...
	    <form id="form1" name="form1" method="post" action="lgplaninfo.do">
		  ...
		  <table width="700" border="0" cellpadding="0" cellspacing="0" class="table">
            ...
            <c:forEach items="${lgtask_plan}" var="g">
              ...
              <td>${g.status }</td>
              <td>${g.isFeedback }</td>
              <td><fmt:formatDate value="${g.beginDate }" pattern="yyyy-MM-dd"/></td>
              <td><fmt:formatDate value="${g.endDate }" pattern="yyyy-MM-dd"/></td>
              <td><label>
                <input type="radio" checked="checked" name="radiobutton" value="${g.planId }-<%=request.getParameter("count")%>" />
              </label></td>
            </tr>
            </c:forEach>
          </table>
