      ...
      <p><a href="../zhuxiao.do" target="_self">退出系统</a></p>
        ...
        <form action="lgtaskinfo.do" method="post" name="form1" target="_self" id="form1">
          <table width="700" border="0" cellpadding="0" cellspacing="0" class="table">
			...
			<tr>
				<td>${g.taskName}</td>
              <td>${g.mp_emp.employeeName}</td>
              <td><fmt:formatDate value="${g.beginDate }" pattern="yyyy-MM-dd"/></td>
              <td><fmt:formatDate value="${g.endDate }" pattern="yyyy-MM-dd"/></td>
              <td>${g.status }</td>
              <td>${g.taskDesc}</td>             	
              <td>
              <input type="hidden" name="taskcount" value="${g.taskDesc}"/>
              <input type="radio" name="radiobutton" value="${g.taskId }-${g.taskDesc}" checked="checked" />
              </td>
              </tr>
			</c:forEach>
          </table>
