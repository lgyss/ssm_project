...
<div id="left">
      ...
      <p ><a href="zhuxiao.do" target="_self">退出系统</a></p>
        ...
        <p>您现在的位置 &gt;&gt; 查询计划 &gt;&gt; 查询</p>
        <form method="post"  action="zuhe_select.do">
...
<input name="endAfter" type="text" size="15" value="${plan_dto.end_date_after}"/></td>
		  ...
		  <table width="700" border="0" cellpadding="0" cellspacing="0" class="table">
            ...
            <tr>
              <td><a href="jihua_selectById.do?jihuaId=${plan.planId}" target="_self">${plan.planName}</a></td>
              <td>${plan.task.taskName}</td>
              <td><fmt:formatDate value="${plan.beginDate}" pattern="yyyy-MM-dd"/></td>
              <td><fmt:formatDate value="${plan.endDate}" pattern="yyyy-MM-dd"/></td>
              <td>${plan.status}</td>
			...
			<a href="zuhe_select.do?PageNo=${page.firstPage}&PageSize=${page.pageSize}">首页</a>
			<a href="zuhe_select.do?PageNo=${page.previousPage}&PageSize=${page.pageSize}">上一页</a>
			<a href="zuhe_select.do?PageNo=${page.nextPage}&PageSize=${page.pageSize}">下一页</a>
			<a href="zuhe_select.do?PageNo=${page.lastPage}&PageSize=${page.pageSize}">末页</a>
			<a>第${page.pageNo }页</a>
