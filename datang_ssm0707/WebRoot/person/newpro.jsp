...
<div id="left">
      ...
      <p ><a href="zhuxiao.do" target="_self">退出系统</a></p>
        ...
        <p>您现在的位置 &gt;&gt; 计划管理 &gt;&gt; 新建计划</p>
                <h1>输入新计划信息</h1>
                <spf:form action="Insertjihua.do?task_id=${taskById.taskId}" modelAttribute="plan" method="post">
          <p>计划名称：
            <label>
            <spf:input path="planName"/>
          </label></p>
          <p>计划描述：
            <label>
            <spf:input path="planDesc"/>
            </label>
          </p>
            <p>开始时间：
              <label>
              	<spf:input path="beginDate"/>
              </label>
             <span class="marginleft1">结束时间：
              <label>
              	<spf:input path="endDate"/>
              </label></span>
          </p>
          <p>计划状态：未完成</p>
          <p>是否反馈：未反馈 </p>
