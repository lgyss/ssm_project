      ...
      <p ><a href="zhuxiao.do" target="_self">退出系统</a></p>
        ...
        <form id="form1" name="form1" method="post" action="renwuById.do">
          <table width="700" border="0" cellpadding="0" cellspacing="0" class="table">
            ...
            <tr>
              <td>${rw.taskName}</td>
              <td>${rw.as_emp.employeeName}</td>
              <td><fmt:formatDate value="${rw.beginDate}" pattern="yyyy-MM-dd"/></td>
              <td><fmt:formatDate value="${rw.endDate}" pattern="yyyy-MM-dd"/></td>
              <td>${rw.status}</td>
              <td>${rw.taskDesc}</td>
              <td><input type="radio" name="jihua_id" value="${rw.taskId}" checked="checked" /></td>
            </tr>
            </c:forEach>
          </table>
