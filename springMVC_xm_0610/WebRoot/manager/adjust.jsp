  ...
  <tr>
    <td class="rightimg"><!-- #BeginLibraryItem "/Library/left.lbi" --><div id="left">
      ...
      <p><a href="../zhuxiao.do" target="_self">退出系统</a></p>
        ...
        <form id="form1" name="form1" method="post" action="lgtaskupdateto.do?taskid=${uptask.task_id}">
          <p>任务名称：<label>
          <input name="task_name" type="text" size="50" value="${uptask.task_name}"/>
          </label></p>
          <p>任务描述：
            <label>
            <textarea name="task_desc">${uptask.task_desc}</textarea>
            </label>
          </p>
            <p>开始时间：
              <label>
                <input name="begin_date" type="text" size="16" value="<fmt:formatDate value="${uptask.begin_date }" pattern="yyyy-MM-dd"/>" />
              </label>
             <span class="marginleft1">结束时间：
              <label>
                <input name="end_date" type="text" size="16" value="<fmt:formatDate value="${uptask.end_date }" pattern="yyyy-MM-dd"/>"/>
              </label></span>
          </p>
          <p>实施人员：
            <label>
              <select name="select">              
                ...
                <c:if test="${g. employee_ID==uptask.mp_emp.employee_ID}">
                <option value="${g. employee_ID}" selected="selected">${g. employee_name}</option>
                </c:if>
                 <c:if test="${g. employee_ID!=uptask.mp_emp.employee_ID}">
